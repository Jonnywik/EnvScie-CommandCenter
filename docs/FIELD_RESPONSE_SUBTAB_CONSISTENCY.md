...
## Narrow-screen header and navigation repair

After the shared header was refactored, legacy mobile CSS still targeted a removed header-workspace element. The stale selector left the Field Response header and contextual navigation vulnerable to wrapping or overlap on narrow screens. The responsive rules now make the contextual navigation a horizontally scrollable flex row at tablet and mobile widths, while the smallest layout reduces the header identity, hides non-essential operator context, truncates the return control, and retains the mode and profile controls inside the continuous header bar.

Post-repair inspection at an emulated `497 × 829` viewport recorded a `499 px` header, `471 px` contextual selector and panel, and **no horizontal overflow**. The rendered Fleet Safety capture showed the compact header controls and four contextual tabs remain separated from the KPI and deployment surfaces. The full frontend validation set passed (**19 tests**), along with TypeScript validation and whitespace checks. Verification only used workspace navigation and visual inspection; it did not invoke dispatch, routing, communications, notifications, resource, SOS, or field-safety state changes.
