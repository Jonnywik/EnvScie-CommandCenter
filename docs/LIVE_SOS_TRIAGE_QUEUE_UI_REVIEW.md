...
|---|---|---|---|---|
| **Resolved P0** | Dispatch proposal integrity | The SOS proposal route, demo path, and Triage Queue now check for an open pending, confirmed, acknowledged, or escalated assignment before team selection or insertion. The database migration adds a partial unique index for the same lifecycle set. | Competing proposals for a single SOS are blocked, including concurrent live requests that bypass the UI. | Apply migration `013_one_open_sos_dispatch_assignment.sql` after resolving any historical competing open records reported by its safety check. The Triage Queue directs the coordinator to review, confirm, or cancel the current lifecycle before another team can be selected. |
...
| One open SOS proposal guard | A second candidate for the same SOS returned `409 Conflict`; after explicit cancellation, a replacement proposal was accepted in the regression suite. |
