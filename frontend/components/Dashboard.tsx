...
import { FacilityVerificationWorkspace } from "./dashboard-tabs/FacilityVerification";
...
import { CommandReadinessBoard, IntelligenceDashboardView, OverviewQuickActions } from "./dashboard-tabs/Intelligence";
import { TriageDrawer } from "./dashboard-tabs/TriageDrawer";
import { LiveSosQueue } from "./dashboard-tabs/LiveSosQueue";
import { AppearanceToggle, type AppearanceMode } from "./dashboard-tabs/AppearanceToggle";
import { CommandCenterHeader } from "./dashboard-tabs/CommandCenterHeader";
import { CommandCenterNavigation, FunctionalViewSelector } from "./dashboard-tabs/CommandCenterNavigation";
import { CommunicationPreflightDialog, FieldResponseAttentionStrip, ResourceAccountabilityDialog, type CommunicationPreflight, type ResourceRequestDraft } from "./dashboard-tabs/FieldResponseControls";
import { FinalizedCommunicationsView, FinalizedResourcesView, FinalizedResponseGroupsView } from "./dashboard-tabs/FieldResponseFinalizedViews";
import type { CommandCenterTab } from "./dashboard-tabs/contracts";


type Tab = CommandCenterTab;
type FieldResponseContext = { groupId?: string; resourceId?: string; incidentId?: string; returnTab?: Tab };
...
export default function Dashboard() {
  ...
  const toggleAppearance = useCallback(() => setAppearance((current) => current === "dark" ? "light" : "dark"), []);
  const [fieldResponseContext, setFieldResponseContext] = useState<FieldResponseContext>(() => {
    if (typeof window === "undefined") return {};
    try { return JSON.parse(window.sessionStorage.getItem("cfr_field_response_context") || "{}"); }
    catch { return {}; }
  });
  const [resourceRequest, setResourceRequest] = useState<{ resource: ResourceItem; action: "resource.resupply_requested" | "resource.reserve_requested" } | null>(null);
  const [communicationPreflight, setCommunicationPreflight] = useState<CommunicationPreflight | null>(null);
  const load = useCallback(async () => {
    ...
    if (summaryResult.status === "fulfilled") {
      ...
      setSelected((current) => current ? summaryResult.value.sos.find((item) => item.id === current.id) || current : null);
    }
  ...
  useEffect(() => { window.sessionStorage.setItem("cfr_field_response_context", JSON.stringify(fieldResponseContext)); }, [fieldResponseContext]);
  useEffect(() => {
    let cancelled = false;
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (shortcut.action === "close") {
        if (document.querySelector("[data-dispatch-team-selector='true']")) return;
        setSelected(null); setManualIntakeOpen(false);
      }
  ...
  const onAction = useCallback(async (action: string, type: string, id?: string, note?: string, metadata?: Record<string, unknown>) => {
    try {
      ...
      const result = await recordOperationsAction({ action, resource_type: type, resource_id: id, note, metadata });
  ...
  const openTriageFromField = (incidentId: string, groupId?: string) => {
    setFieldResponseContext((current) => ({ ...current, incidentId, groupId: groupId || current.groupId, returnTab: tab }));
    const incident = incidents.find((item) => item.id === incidentId);
    if (incident) setSelected(incident);
    setTab("Incident Triage");
  };
  const recordResourceRequest = async (draft: ResourceRequestDraft) => {
    if (!resourceRequest) return;
    const { resource, action } = resourceRequest;
    const requestKind = action === "resource.resupply_requested" ? "resupply" : "reserve";
    await onAction(action, "resource", resource.id, `Recorded ${requestKind} request for ${resource.name}; requested ${draft.amount} ${resource.unit}; owner ${draft.owner}; review by ${draft.reviewAt}.`, { requested_amount: draft.amount, unit: resource.unit, responsible_owner: draft.owner, rationale: draft.rationale, review_at: draft.reviewAt, reported_available: resource.available, reported_total: resource.total, decision_limit: "Request recorded for human follow-up; no stock movement, delivery, reservation, or availability is confirmed." });
    setToast(`${resource.name}: ${requestKind} request recorded for review.`);
    setResourceRequest(null);
  };
  const confirmCommunicationPreflight = async () => {
    if (!communicationPreflight) return;
    const draft = communicationPreflight;
    await sendCoordinationCommunication({ channel: draft.channel, to_unit: draft.toUnit, message: draft.message, priority: draft.priority, linked_incident_id: draft.incidentId, simulate_audio: draft.simulateAudio });
    await onAction("coordination.message_sent", "communication_event", undefined, `Recorded ${draft.priority} ${draft.channel} communication to ${draft.toUnit}.`, { recipient_scope: draft.toUnit, channel: draft.channel, priority: draft.priority, linked_incident_id: draft.incidentId, simulation_clip: draft.simulateAudio, decision_limit: "A communication event does not confirm receipt, field acknowledgement, route clearance, or responder safety." });
    setToast(`Communication recorded for ${draft.toUnit}. Confirm delivery and field acknowledgement separately.`);
    setCommunicationPreflight(null);
    await load();
  };
  if (["Response Groups", "Resources", "Communications"].includes(tab)) return <div className="workspace-navigation-shell"><CommandCenterNavigation activeTab={tab} onNavigate={setTab} badges={{ Incidents: incidents.filter((incident) => incident.status === "received").length || undefined, "Field Response": notifications.pending_count || undefined }} /><main className="field-response-context min-h-screen p-3 sm:p-5" aria-label="Field Response operational workspace"><CommandCenterHeader title={`Field Response / ${tab === "Response Groups" ? "Teams & Assets" : tab === "Resources" ? "Resource Logistics" : "Communications"}`} context="COMMAND CENTER · FIELD READINESS, ACCOUNTABILITY & COORDINATION" appearance={appearance} onAppearanceChange={toggleAppearance} operatorLabel="Duty officer · field response" syncLabel={`Snapshot ${formatAge(responseGroups.generated_at)}`} onReturn={() => setTab("Overview")} /><FunctionalViewSelector activeTab={tab} onNavigate={setTab} className="mx-auto mt-4 max-w-[1800px]" /><div className="mx-auto max-w-[1800px]"><FieldResponseAttentionStrip groups={responseGroups} resources={operations.resources} communications={coordination} notificationSnapshot={notifications} onNavigate={setTab} />{tab === "Response Groups" && <FinalizedResponseGroupsView snapshot={responseGroups} incidents={incidents} initialSelectedId={fieldResponseContext.groupId} onSelectedGroupChange={(groupId) => setFieldResponseContext((current) => ({ ...current, groupId }))} onOpenTriage={openTriageFromField} />}{tab === "Resources" && <FinalizedResourcesView resources={operations.resources} communications={operations.communications} initialSelectedId={fieldResponseContext.resourceId} onSelectedResourceChange={(resourceId) => setFieldResponseContext((current) => ({ ...current, resourceId }))} onOpenRequest={(resource, action) => { setFieldResponseContext((current) => ({ ...current, resourceId: resource.id })); setResourceRequest({ resource, action }); }} onAction={onAction} />}{tab === "Communications" && <FinalizedCommunicationsView communications={coordination} incidents={incidents} groups={responseGroups} notifications={notifications} onPrepareDispatch={setCommunicationPreflight} onOpenTriage={openTriageFromField} />}</div>{resourceRequest && <ResourceAccountabilityDialog resource={resourceRequest.resource} action={resourceRequest.action} onClose={() => setResourceRequest(null)} onSubmit={recordResourceRequest} />}{communicationPreflight && <CommunicationPreflightDialog draft={communicationPreflight} channelHealth={coordination.channel_health} onClose={() => setCommunicationPreflight(null)} onConfirm={confirmCommunicationPreflight} />}</main>{toast && <div className="action-toast" role="status"><span>✓</span>{toast}</div>}</div>;
  if (tab === "Overview") return <><CommandMapView summary={summary} gis={gis} groups={responseGroups} health={feedHealth} user={user} connection={connection} appearance={appearance} onAppearanceChange={toggleAppearance} onSelect={setSelected} onNavigate={setTab} onAction={onAction} onRefresh={() => void load()} error={error} />{selected && <TriageDrawer incident={selected} onClose={() => setSelected(null)} onUpdated={(updated) => setSummary((current) => current ? { ...current, sos: current.sos.map((item) => item.id === updated.id ? updated : item) } : current)} onAction={onAction} />}{manualIntakeOpen && <CoordinatorEmergencyDrawer onClose={() => setManualIntakeOpen(false)} onCreated={handleCoordinatorEmergencyCreated} />}{toast && <div className="action-toast" role="status"><span>✓</span>{toast}</div>}</>;
  if (tab === "Incident Triage") return <IncidentTriageView incidents={incidents} alerts={alerts} appearance={appearance} onAppearanceChange={toggleAppearance} onAction={onAction} onRefresh={load} onReturn={() => setTab(fieldResponseContext.returnTab || "Overview")} onNavigate={setTab} selectedIncidentId={selected?.id} preferredGroupId={fieldResponseContext.groupId} />;
  if (tab === "Fleet & Responder Safety") return <FleetResponderSafetyView groups={responseGroups} gis={gis} appearance={appearance} onAppearanceChange={toggleAppearance} onAction={onAction} onRefresh={load} onReturn={() => setTab("Overview")} onNavigate={setTab} selectedGroupId={fieldResponseContext.groupId} onSelectedGroupChange={(groupId) => setFieldResponseContext((current) => ({ ...current, groupId }))} />;
