...
function OverviewView({ summary, incidents, centers, alerts, health, gis, groups, notifications, onSelect, onAction }: { summary: DashboardSummary; incidents: SosIncident[]; centers: Center[]; alerts: AlertItem[]; health: FeedHealth[]; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; notifications: NotificationSnapshot; onSelect: (incident: SosIncident) => void; onAction: (action: string, type: string, id?: string, note?: string) => Promise<void> }) {
  return <><div className="metric-grid"><MetricCard label="Untriaged SOS" value={summary.metrics.untriaged_sos} note="Life-safety queue needs a decision" icon="!" bg="#ffe4e6" /><MetricCard label="Critical alerts" value={summary.metrics.critical_alerts} note="Verified warning areas" icon="◉" bg="#ffedd5" /><MetricCard label="Open centers" value={summary.metrics.open_centers} note="Capacity snapshot verified" icon="⌂" bg="#ccfbf1" /><MetricCard label="Residents at risk" value={summary.metrics.residents_at_risk} note="Based on active polygons" icon="⌁" bg="#e0f2fe" /></div><CommandReadinessBoard incidents={incidents} centers={centers} health={health} groups={groups} notifications={notifications} /><div className="content-grid"><div className="left-stack"><GISMapPanel snapshot={gis} onAction={onAction} /><SosQueue incidents={incidents} onSelect={onSelect} /></div><div className="right-stack"><AlertFeed alerts={alerts} health={health} onAction={onAction} /><CenterList centers={centers} onAction={onAction} /></div></div></>;
}
