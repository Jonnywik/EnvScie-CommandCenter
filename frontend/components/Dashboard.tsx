...
function ObjectiveBoard({ objectives, tasks, teams }: { objectives: string[]; tasks: CommandCenterTask[]; teams: DispatchTeam[] }) {
  return <div className="operations-grid three-columns"><section className="panel"><PanelHeader title="Incident objectives" subtitle="Commander’s intent for this operating period" /><div className="panel-body objective-list">{objectives.map((objective, index) => <div className="objective-row" key={objective}><span>{String(index + 1).padStart(2, "0")}</span><p>{objective}</p></div>)}</div></section><section className="panel"><PanelHeader title="Operational task board" subtitle="Owner, due time, and blockers" /><div className="panel-body task-list">{tasks.map((task) => <div className="task-row" key={task.id}><div><strong>{task.title}</strong><span>{task.owner} · due {task.due_by}</span>{task.dependency && <small>Depends on: {task.dependency}</small>}</div><span className={`badge ${severityClass(task.priority)} ${stateClass(task.status)}`}>{task.status.replace("_", " ")}</span></div>)}</div></section><section className="panel"><PanelHeader title="Responder posture" subtitle="Teams and communications status" /><div className="panel-body team-list">{teams.map((team) => <div className="team-row" key={team.id}><div className="team-avatar">{team.name.slice(0, 1)}</div><div><strong>{team.name}</strong><span>{team.mission} · {team.members} people</span><small>{team.vehicle} · {team.channel}</small></div><span className={`badge ${stateClass(team.status)}`}>{team.status.replace("_", " ")}</span></div>)}</div></section></div>;
}
...
function CommandReadinessBoard({ incidents, centers, health, groups, notifications }: { incidents: SosIncident[]; centers: Center[]; health: FeedHealth[]; groups: ResponseGroupSnapshot; notifications: NotificationSnapshot }) {
  ...
  const controls = [
    { label: "Unacknowledged SOS", value: incidents.filter((item) => item.status === "received").length, state: "critical", note: "Triage before dispatch" },
    { label: "Stale response groups", value: staleGroups, state: staleGroups ? "warning" : "good", note: "Refresh both position and roster check-in" },
    { label: "Stale official feeds", value: health.filter((item) => item.stale).length, state: health.some((item) => item.stale) ? "warning" : "good", note: "Synchronize before issuing new public guidance" },
    { label: "Constrained centers", value: constrainedCenters, state: constrainedCenters ? "warning" : "good", note: "Review capacity before moving residents" },
    { label: "Undelivered assignments", value: notifications.pending_count + notifications.failed_count, state: notifications.pending_count + notifications.failed_count ? "warning" : "good", note: "Confirm field acknowledgement by alternate channel" },
  ];
  return <section className="panel readiness-board"><PanelHeader title="Command readiness board" subtitle="Cross-system guardrails that must be reviewed before a new deployment or public movement order" action={<span className="panel-link">Decision support, not auto-clearance</span>} /><div className="readiness-list">{controls.map((control) => <div className="command-readiness-row" key={control.label}><span className={`readiness-state ${control.state}`} /><div><strong>{control.label}</strong><small>{control.note}</small></div><b>{control.value}</b></div>)}</div></section>;
}

function OverviewView({ summary, operations, incidents, centers, alerts, health, gis, groups, notifications, onSelect, onAction }: { summary: DashboardSummary; operations: OperationsSnapshot; incidents: SosIncident[]; centers: Center[]; alerts: AlertItem[]; health: FeedHealth[]; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; notifications: NotificationSnapshot; onSelect: (incident: SosIncident) => void; onAction: (action: string, type: string, id?: string, note?: string) => Promise<void> }) {
  return <><div className="metric-grid"><MetricCard label="Untriaged SOS" value={summary.metrics.untriaged_sos} note="Life-safety queue needs a decision" icon="!" bg="#ffe4e6" /><MetricCard label="Critical alerts" value={summary.metrics.critical_alerts} note="Verified warning areas" icon="◉" bg="#ffedd5" /><MetricCard label="Open centers" value={summary.metrics.open_centers} note="Capacity snapshot verified" icon="⌂" bg="#ccfbf1" /><MetricCard label="Residents at risk" value={summary.metrics.residents_at_risk} note="Based on active polygons" icon="⌁" bg="#e0f2fe" /></div><CommandReadinessBoard incidents={incidents} centers={centers} health={health} groups={groups} notifications={notifications} /><ObjectiveBoard objectives={operations.objectives} tasks={operations.tasks} teams={operations.teams} /><div className="content-grid"><div className="left-stack"><GISMapPanel snapshot={gis} onAction={onAction} /><SosQueue incidents={incidents} onSelect={onSelect} /></div><div className="right-stack"><AlertFeed alerts={alerts} health={health} onAction={onAction} /><CenterList centers={centers} onAction={onAction} /></div></div></>;
}
