...
import { CommandCenterHeaderIdentity } from "./CommandCenterHeader";
...
export function CommandMapView({ summary, gis, groups, health, user, connection, appearance, onAppearanceChange, onSelect, onNavigate, onAction, onRefresh, error }: { summary: DashboardSummary; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; health: FeedHealth[]; user: UserIdentity | null; connection: "live" | "cached"; appearance: AppearanceMode; onAppearanceChange: () => void; onSelect: (incident: SosIncident) => void; onNavigate: (tab: CommandCenterTab) => void; onAction: OperationalAction; onRefresh: () => void; error: string | null }) {
  ...
  return <div className="workspace-navigation-shell command-map-workspace radar-removed"><CommandCenterNavigation activeTab="Overview" onNavigate={onNavigate} className="command-map-sidebar" overlayOnExpand /><main className={`command-map-shell appearance-${appearance}`} aria-label="Command Map Live Operations">
    ...
    <header className="command-map-topbar">
      <CommandCenterHeaderIdentity className="command-map-brand" context="COMMAND MAP · LIVE OPERATIONS" timeLabel={`${commandTime} PST`} timeDateTime={now.toISOString()} />
      ...
      <div className="command-map-session"><div className="command-map-tools"><button type="button" className="command-map-tools-trigger" aria-expanded={mapToolsOpen} aria-controls="command-map-tools-menu" onClick={() => setMapToolsOpen((open) => !open)}><span aria-hidden="true">▤</span>Map tools</button>{mapToolsOpen && <div id="command-map-tools-menu" className="command-map-tools-menu" role="group" aria-label="Map tools"><button type="button" className={activeUtilityPanel === "view" ? "active" : ""} onClick={() => selectMapTool("view")}>Map view</button><button type="button" className={activeUtilityPanel === "triage" ? "active" : ""} onClick={() => selectMapTool("triage")}>Triage</button><button type="button" className={activeUtilityPanel === "facilities" ? "active" : ""} onClick={() => selectMapTool("facilities")}>Facilities</button><button type="button" className={activeUtilityPanel === "sources" ? "active" : ""} onClick={() => selectMapTool("sources")}>Sources</button></div>}</div><div className={`system-health ${systemHealthy ? "healthy" : "degraded"}`}><span />{systemHealthy ? "Sources in policy" : "Review source health"}</div><button className="command-avatar" type="button" aria-label="Current operator profile">{user?.display_name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase() || "DR"}</button><AppearanceToggle appearance={appearance} onAppearanceChange={onAppearanceChange} /></div>
    </header>
