...
type CommandMapUtilityPanel = "view" | "triage" | "facilities" | "sources" | null;
...
export function GISMapPanel({ snapshot, route, onAction, variant = "panel", commandLayers, layerTool, filterTool, activeUtilityPanel = null, onActiveUtilityPanelChange, renderFacilityTrigger = true, appearance = "dark", mapBasemap, noahContext, onOpenIncidentWorkspace, onOpenResourceWorkspace, onOpenCenterWorkspace }: { snapshot: GisMapSnapshot; route?: OptimizedRoute | null; onAction: OperationalAction; variant?: "panel" | "command"; commandLayers?: CommandMapLayers; layerTool?: React.ReactNode; filterTool?: React.ReactNode; activeUtilityPanel?: CommandMapUtilityPanel; onActiveUtilityPanelChange?: (panel: CommandMapUtilityPanel) => void; renderFacilityTrigger?: boolean; appearance?: "dark" | "light"; mapBasemap?: GoogleBasemap; noahContext?: NoahMapContext | null; onOpenIncidentWorkspace?: (id: string) => void; onOpenResourceWorkspace?: (id: string) => void; onOpenCenterWorkspace?: (id: string) => void }) {
  ...
  const facilityControlOpen = activeUtilityPanel === "facilities";
  ...
  const fallbackSos = useMemo(() => [...snapshot.sos].sort((left, right) => {
    const leftPriority = left.id === hoveredSosId || (selectedPin?.kind === "sos" && selectedPin.item.id === left.id) ? 1 : 0;
    const rightPriority = right.id === hoveredSosId || (selectedPin?.kind === "sos" && selectedPin.item.id === right.id) ? 1 : 0;
    return leftPriority - rightPriority;
  }), [hoveredSosId, selectedPin, snapshot.sos]);
  ...
  return <section className={`panel map-panel gis-map-panel ${commandMapMode ? "command-map-hero" : ""}`}>
    ...
    <div ref={mapShellRef} className="gis-map-shell" tabIndex={0} aria-label="Interactive Google Maps viewport. Drag to pan; use the mouse wheel or controls to zoom." onWheel={googleMap ? undefined : onWheel} onPointerDown={googleMap ? undefined : onPointerDown} onPointerMove={googleMap ? undefined : onPointerMove} onPointerUp={googleMap ? undefined : endPointer} onPointerCancel={googleMap ? undefined : endPointer} onTouchStart={googleMap ? undefined : onTouchStart} onTouchMove={googleMap ? undefined : onTouchMove} onTouchEnd={googleMap ? undefined : onTouchEnd}>
      ...
      {(!googleMapTilesReady || googleMapError) && <svg className={`gis-map-canvas ${displayedBasemap === "satellite" ? "satellite" : "operational"} ${commandMapMode ? "command-map-canvas" : ""}`} viewBox={`0 0 ${width} ${height}`} role="img" aria-label={regionalContext ? "Satellite and operational regional map showing Balangiga, Eastern Samar, Leyte Gulf, and surrounding provincial context" : "Satellite and operational map of Balangiga response resources and evacuation constraints"}>
        ...
        {displayedLayers.sos && fallbackSos.map((incident) => { const p = point(incident.position); const selected = selectedPin?.kind === "sos" && selectedPin.item.id === incident.id; const hovered = hoveredSosId === incident.id; const revealLabel = incident.severity === "critical" && (selected || hovered); const select = () => { setHoveredSosId(null); setSelectedPin({ kind: "sos", item: incident }); }; const activeLabel = `Reported SOS · ${incident.summary}`.slice(0, 34); return <g key={incident.id} className={`gis-marker gis-sos-marker ${selected ? "selected" : ""} ${hovered ? "hovered" : ""}`} transform={`translate(${p.x} ${p.y})`} onClick={select} onPointerEnter={() => incident.severity === "critical" && setHoveredSosId(incident.id)} onPointerLeave={() => setHoveredSosId((current) => current === incident.id ? null : current)} onFocus={() => incident.severity === "critical" && setHoveredSosId(incident.id)} onBlur={() => setHoveredSosId((current) => current === incident.id ? null : current)} onKeyDown={(event) => pinKeyActivate(event, select)} role="button" tabIndex={0} aria-label={`Inspect SOS alert ${incident.id}`}><circle r="9" /><text x="0" y="4" textAnchor="middle">!</text>{revealLabel && <g className={`gis-critical-sos-label ${selected ? "is-active" : "is-hovered"}`} pointerEvents="none"><rect x="13" y="-23" width="154" height="20" rx="5" /><text x="20" y="-10">{activeLabel}</text></g>}<title>{incident.summary} · {incident.status}</title></g>; })}
...
export function CommandMapView({ summary, gis, groups, health, user, connection, appearance, onAppearanceChange, onSelect, onNavigate, onAction, onRefresh, error }: { summary: DashboardSummary; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; health: FeedHealth[]; user: UserIdentity | null; connection: "live" | "cached"; appearance: AppearanceMode; onAppearanceChange: () => void; onSelect: (incident: SosIncident) => void; onNavigate: (tab: CommandCenterTab) => void; onAction: OperationalAction; onRefresh: () => void; error: string | null }) {
  ...
  const [activeUtilityPanel, setActiveUtilityPanel] = useState<CommandMapUtilityPanel>(null);
  ...
  useEffect(() => {
    const closeUtilityPanel = (event: KeyboardEvent) => { if (event.key === "Escape") { setActiveUtilityPanel(null); setMapToolsOpen(false); } };
  ...
  const systemHealthy = connection === "live" && health.every((item) => !item.stale) && gis.source_health.every((item) => item.status === "healthy" || item.status === "reference_only");
  ...
  const toggleUtilityPanel = (panel: Exclude<CommandMapUtilityPanel, null>) => setActiveUtilityPanel((current) => current === panel ? null : panel);
  const selectMapTool = (panel: Exclude<CommandMapUtilityPanel, null>) => { setActiveUtilityPanel((current) => current === panel ? null : panel); setMapToolsOpen(false); };
  ...
  const reviewSource = async (sourceId: string) => {
    try {
      const result = await reviewMapSource(sourceId);
      await onAction("gis.source_health_reviewed", "map_source", sourceId, `Human review recorded at ${formatTime(result.reviewed_at)}; ${result.decision_limit}`);
      onRefresh();
    } catch (reviewError) {
      const note = reviewError instanceof Error ? reviewError.message : "Unable to record source review.";
      await onAction("gis.source_health_review_failed", "map_source", sourceId, note);
    }
  };
  return <div className="workspace-navigation-shell command-map-workspace radar-removed"><CommandCenterNavigation activeTab="Overview" onNavigate={onNavigate} /><main className={`command-map-shell appearance-${appearance}`} aria-label="Command Map Live Operations">
    <GISMapPanel snapshot={filteredGis} onAction={onAction} variant="command" appearance={appearance} commandLayers={layers} mapBasemap={basemap} noahContext={noahContext} activeUtilityPanel={activeUtilityPanel} onActiveUtilityPanelChange={setActiveUtilityPanel} renderFacilityTrigger={false} layerTool={<CommandMapLayerTool layers={layers} toggleLayer={toggleLayer} basemap={basemap} setBasemap={setBasemap} noahContext={noahContext} noahError={noahError} open={activeUtilityPanel === "view"} onToggle={() => toggleUtilityPanel("view")} renderTrigger={false} />} filterTool={<><CommandMapTriageFilterTool filters={triageFilters} onChange={setTriageFilters} onClear={() => setTriageFilters(commandMapFilterDefaults)} summary={filterSummary} open={activeUtilityPanel === "triage"} onToggle={() => toggleUtilityPanel("triage")} renderTrigger={false} /><CommandMapSourceHealthTool sources={gis.source_health} onReview={(sourceId) => void reviewSource(sourceId)} open={activeUtilityPanel === "sources"} onToggle={() => toggleUtilityPanel("sources")} renderTrigger={false} /></>} onOpenIncidentWorkspace={(id) => { const incident = summary.sos.find((item) => item.id === id); if (incident) onSelect(incident); onNavigate("Incident Triage"); }} onOpenResourceWorkspace={() => onNavigate("Fleet & Responder Safety")} onOpenCenterWorkspace={() => onNavigate("Evacuation Centers")} />
    <CommandCenterHeader title="Command Map / Live Operations" context="COMMAND CENTER · LIVE OPERATIONS" appearance={appearance} onAppearanceChange={onAppearanceChange} operatorLabel={user ? `${user.display_name} · ${user.role}` : "Operational session"} syncLabel={`${commandTime} PST · ${systemHealthy ? "sources in policy" : "source review required"}`} connection={connection === "cached" ? "cached" : systemHealthy ? "live" : "attention"} className="command-map-topbar" centerContent={<div className="command-search-wrap"><form className="command-search" onSubmit={runSearch}><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => { setQuery(event.target.value); setSearchStatus(null); }} placeholder="Search coordinates, LGU zone, responder ID" aria-label="Search coordinates, LGU zones, or responder IDs" /><button type="submit">Search</button></form>{normalizedQuery && <div className="command-search-results" role="status">{coordinateQuery && <button type="button" onClick={() => setSearchStatus(`Coordinates located: ${query.trim()}`)}>◎ Coordinates · {query.trim()}</button>}{matchingIncidents.map((incident) => <button type="button" key={incident.id} onClick={() => onSelect(incident)}>! SOS · {incident.emergency_type} · {incident.barangay}</button>)}{matchingGroups.map((group) => <button type="button" key={group.id} onClick={() => setSearchStatus(`${group.name} is available in Field response.`)}>◎ Unit · {group.call_sign} · {group.name}</button>)}{matchingZones.map((zone) => <button type="button" key={zone} onClick={() => setSearchStatus(`${zone} is within the active operational map extent.`)}>⌖ LGU zone · {zone}</button>)}{!coordinateQuery && !matchingIncidents.length && !matchingGroups.length && !matchingZones.length && <span>No matching operational record.</span>}</div>}</div>} actions={<div className="command-map-tools"><button type="button" className="command-map-tools-trigger" aria-expanded={mapToolsOpen} aria-controls="command-map-tools-menu" onClick={() => setMapToolsOpen((open) => !open)}><span aria-hidden="true">▤</span>Map tools</button>{mapToolsOpen && <div id="command-map-tools-menu" className="command-map-tools-menu" role="group" aria-label="Map tools"><button type="button" className={activeUtilityPanel === "view" ? "active" : ""} onClick={() => selectMapTool("view")}>Map view</button><button type="button" className={activeUtilityPanel === "triage" ? "active" : ""} onClick={() => selectMapTool("triage")}>Triage</button><button type="button" className={activeUtilityPanel === "facilities" ? "active" : ""} onClick={() => selectMapTool("facilities")}>Facilities</button><button type="button" className={activeUtilityPanel === "sources" ? "active" : ""} onClick={() => selectMapTool("sources")}>Sources</button></div>}</div>} />
