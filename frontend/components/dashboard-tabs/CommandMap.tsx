...
type CommandMapUtilityPanel = "view" | "triage" | "facilities" | null;
...
export function CommandMapView({ summary, gis, groups, health, user, connection, appearance, onAppearanceChange, onSelect, onNavigate, onAction, onRefresh, error }: { summary: DashboardSummary; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; health: FeedHealth[]; user: UserIdentity | null; connection: "live" | "cached"; appearance: AppearanceMode; onAppearanceChange: () => void; onSelect: (incident: SosIncident) => void; onNavigate: (tab: CommandCenterTab) => void; onAction: OperationalAction; onRefresh: () => void; error: string | null }) {
  ...
  return <div className={`workspace-navigation-shell command-map-workspace ${radarMinimized ? "radar-minimized" : "radar-expanded"}`}><CommandCenterNavigation activeTab="Overview" onNavigate={onNavigate} className="command-map-sidebar" overlayOnExpand /><main className={`command-map-shell appearance-${appearance}`} aria-label="Command Map Live Operations">
    ...
    <section className={`responder-radar ${radarMinimized ? "is-minimized" : ""}`} aria-label="Responder radar"><div className="responder-radar-heading"><div><span>FIELD UNITS</span><h2>{radarMinimized ? `${groups.groups.filter((group) => group.status !== "offline").length} active field units` : "Responder radar"}</h2></div><div className="responder-radar-actions"><button type="button" onClick={() => onNavigate("Response Groups")}>Open roster →</button><button type="button" className="responder-radar-toggle" aria-expanded={!radarMinimized} aria-label={radarMinimized ? "Expand Responder radar" : "Minimize Responder radar"} onClick={() => setRadarMinimized((minimized) => !minimized)}><span aria-hidden="true">{radarMinimized ? "⌃" : "⌄"}</span></button></div></div>{!radarMinimized && <div className="responder-carousel">{groups.groups.filter((group) => group.status !== "offline").slice(0, 8).map((group) => <button type="button" className={`responder-radar-card ${group.status}`} key={group.id} onClick={() => setSearchStatus(`${group.name} · ${group.location_label} · last updated ${formatAge(group.last_location_at)}`)}><span className="responder-status-dot" /><strong>{group.name}</strong><small>{group.vehicle_or_asset || group.group_type}</small><div><span>{group.status.replaceAll("_", " ")}</span><b>{group.estimated_response_minutes ? `${group.estimated_response_minutes} min` : "ETA —"}</b></div></button>)}{!groups.groups.length && <div className="command-empty">No responder groups are reporting.</div>}</div>}</section>
