...
import { type AlertItem, type Center, type DashboardSummary, type FeedHealth, type GisMapSnapshot, type GisResource, type MapOverlaysSnapshot, type NoahMapContext, type OptimizedRoute, type RadarSnapshot, type ResponseGroupSnapshot, type SosIncident, type TyphoonSnapshot, type UserIdentity, getMapOverlays, getNoahMapContext, updateGisResourcePosition } from "../../lib/api";
...
export function GISMapPanel({ snapshot, route, onAction, variant = "panel", commandLayers, layerTool, appearance = "dark", mapBasemap, noahContext }: { snapshot: GisMapSnapshot; route?: OptimizedRoute | null; onAction: OperationalAction; variant?: "panel" | "command"; commandLayers?: CommandMapLayers; layerTool?: React.ReactNode; appearance?: "dark" | "light"; mapBasemap?: GoogleBasemap; noahContext?: NoahMapContext | null }) {
  ...
  return <section className={`panel map-panel gis-map-panel ${commandMapMode ? "command-map-hero" : ""}`}>
    ...
    <div ref={mapShellRef} className="gis-map-shell" tabIndex={0} aria-label="Interactive Google Maps viewport. Drag to pan; use the mouse wheel or controls to zoom." onWheel={googleMap ? undefined : onWheel} onPointerDown={googleMap ? undefined : onPointerDown} onPointerMove={googleMap ? undefined : onPointerMove} onPointerUp={googleMap ? undefined : endPointer} onPointerCancel={googleMap ? undefined : endPointer} onTouchStart={googleMap ? undefined : onTouchStart} onTouchMove={googleMap ? undefined : onTouchMove} onTouchEnd={googleMap ? undefined : onTouchEnd}>
      <GoogleOperationalMap snapshot={snapshot} route={route} layers={displayedLayers} radar={radar} typhoon={typhoon} mapOverlays={mapOverlays} noahContext={noahContext || null} appearance={appearance} basemap={googleBasemap} onSelectResource={(item) => setSelectedPin({ kind: "resource", item })} onSelectCenter={(item) => setSelectedPin({ kind: "center", item })} onSelectSos={(item) => setSelectedPin({ kind: "sos", item })} onReady={(map) => { setGoogleMap(map); setGoogleMapError(null); }} onError={setGoogleMapError} />
