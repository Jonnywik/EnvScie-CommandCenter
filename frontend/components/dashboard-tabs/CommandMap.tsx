...
import { type AlertItem, type Center, type DashboardSummary, type FeedHealth, type GisMapSnapshot, type GisResource, type MapOverlaysSnapshot, type OptimizedRoute, type RadarSnapshot, type ResponseGroupSnapshot, type SosIncident, type TyphoonSnapshot, type UserIdentity, getMapOverlays, updateGisResourcePosition } from "../../lib/api";
...
type CommandMapLayers = { weatherRadar: boolean; floodRisk: boolean; pagasaRadar: boolean; pagasaStations: boolean; pagasaSatellite: boolean; lightning: boolean };
...
export function GISMapPanel({ snapshot, route, onAction, variant = "panel", commandLayers, layerTool, appearance = "dark", mapBasemap }: { snapshot: GisMapSnapshot; route?: OptimizedRoute | null; onAction: OperationalAction; variant?: "panel" | "command"; commandLayers?: CommandMapLayers; layerTool?: React.ReactNode; appearance?: "dark" | "light"; mapBasemap?: GoogleBasemap }) {
  const [layers, setLayers] = useState({ hazards: true, resources: true, sos: true, centers: true, route: true, radar: true, typhoon: true, pagasaRadar: false, pagasaStations: false, pagasaSatellite: false, lightning: false });
  ...
  return <section className={`panel map-panel gis-map-panel ${commandMapMode ? "command-map-hero" : ""}`}>
    ...
    <div ref={mapShellRef} className="gis-map-shell" tabIndex={0} aria-label="Interactive Google Maps viewport. Drag to pan; use the mouse wheel or controls to zoom." onWheel={googleMap ? undefined : onWheel} onPointerDown={googleMap ? undefined : onPointerDown} onPointerMove={googleMap ? undefined : onPointerMove} onPointerUp={googleMap ? undefined : endPointer} onPointerCancel={googleMap ? undefined : endPointer} onTouchStart={googleMap ? undefined : onTouchStart} onTouchMove={googleMap ? undefined : onTouchMove} onTouchEnd={googleMap ? undefined : onTouchEnd}>
      <GoogleOperationalMap snapshot={snapshot} route={route} layers={displayedLayers} radar={radar} typhoon={typhoon} mapOverlays={mapOverlays} appearance={appearance} basemap={googleBasemap} onSelectResource={(item) => setSelectedPin({ kind: "resource", item })} onSelectCenter={(item) => setSelectedPin({ kind: "center", item })} onSelectSos={(item) => setSelectedPin({ kind: "sos", item })} onReady={(map) => { setGoogleMap(map); setGoogleMapError(null); }} onError={setGoogleMapError} />
      {googleMapError && <svg className={`gis-map-canvas ${displayedBasemap === "satellite" ? "satellite" : "operational"} ${commandMapMode ? "command-map-canvas" : ""}`} viewBox={`0 0 ${width} ${height}`} role="img" aria-label={regionalContext ? "Satellite and operational regional map showing Balangiga, Eastern Samar, Leyte Gulf, and surrounding provincial context" : "Satellite and operational map of Balangiga response resources and evacuation constraints"}>
      ...
      <div className="gis-map-badge"><span className={`health-dot ${weatherError || googleMapError || radar?.stale || typhoon?.stale ? "stale" : ""}`} />{googleMapError || weatherError || (radar?.frames.length ? `Radar ${radar.stale ? "cached" : "updated"} ${formatAge(radar.fetched_at)} · regional scale ≤10` : "Radar unavailable")} · PAGASA/QPE pending approval · Lightning provider pending · {googleBasemap === "satellite" ? "Satellite imagery" : googleBasemap === "terrain" ? "Terrain context" : "Road map"}</div>
...
export function CommandMapView({ summary, gis, groups, health, user, connection, appearance, onAppearanceChange, onSelect, onNavigate, onAction, onRefresh, error }: { summary: DashboardSummary; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; health: FeedHealth[]; user: UserIdentity | null; connection: "live" | "cached"; appearance: AppearanceMode; onAppearanceChange: () => void; onSelect: (incident: SosIncident) => void; onNavigate: (tab: CommandCenterTab) => void; onAction: OperationalAction; onRefresh: () => void; error: string | null }) {
  const [layers, setLayers] = useState<CommandMapLayers>({ weatherRadar: true, floodRisk: true, pagasaRadar: false, pagasaStations: false, pagasaSatellite: false, lightning: false });
  ...
  return <div className="workspace-navigation-shell command-map-workspace"><CommandCenterNavigation activeTab="Overview" onNavigate={onNavigate} className="command-map-sidebar" /><main className={`command-map-shell appearance-${appearance}`} aria-label="Command Map Live Operations">
    <GISMapPanel snapshot={gis} onAction={onAction} variant="command" appearance={appearance} commandLayers={layers} mapBasemap={basemap} layerTool={<section className={`command-map-layer-rail ${layerDrawerOpen ? "is-open" : ""}`} aria-label="Map layer controls"><button className="command-map-layer-trigger" type="button" onClick={toggleLayerDrawer} aria-expanded={layerDrawerOpen} aria-controls="command-map-layer-drawer" aria-label={layerDrawerOpen ? "Close map layers" : "Open map layers"} title={layerDrawerOpen ? "Close map layers" : "Open map layers"}><span aria-hidden="true">▤</span></button>{layerDrawerOpen && <div id="command-map-layer-drawer" className="command-map-layer-drawer"><div className="command-map-layer-heading"><span>MAP VIEW</span><b>Basemap &amp; operational overlays</b></div><div className="command-map-basemap-controls" role="group" aria-label="Google Maps basemap"><button type="button" className={basemap === "roadmap" ? "active" : ""} aria-pressed={basemap === "roadmap"} onClick={() => setBasemap("roadmap")}>Road</button><button type="button" className={basemap === "satellite" ? "active" : ""} aria-pressed={basemap === "satellite"} onClick={() => setBasemap("satellite")}>Satellite</button><button type="button" className={basemap === "terrain" ? "active" : ""} aria-pressed={basemap === "terrain"} onClick={() => setBasemap("terrain")}>Terrain</button></div><div className="layer-switch-group"><span>ACTIVE CONTEXT</span><div className="layer-switches">{([{ key: "weatherRadar", label: "Live Weather Radar", accent: "violet" }, { key: "floodRisk", label: "Flood Risk Zones", accent: "coral" }] as const).map((layer) => <label className="layer-switch" key={layer.key}><span><i className={layer.accent} />{layer.label}</span><button type="button" role="switch" aria-checked={layers[layer.key]} onClick={() => toggleLayer(layer.key)}><b /></button></label>)}</div></div><div className="layer-switch-group"><span>LIVE METEOROLOGY</span><div className="layer-switches">{([{ key: "pagasaRadar", label: "PAGASA Radar/QPE", detail: "Provider access pending", accent: "teal" }, { key: "pagasaStations", label: "PAGASA Stations", detail: "Provider access pending", accent: "teal" }, { key: "pagasaSatellite", label: "Himawari Context", detail: "Provider access pending", accent: "teal" }, { key: "lightning", label: "Licensed Lightning", detail: "Provider procurement pending", accent: "amber" }] as const).map((layer) => <label className="layer-switch pending" key={layer.key}><span><i className={layer.accent} /><span>{layer.label}<small>{layer.detail}</small></span></span><button type="button" role="switch" aria-checked={false} aria-describedby={`${layer.key}-status`} disabled><b /></button><small id={`${layer.key}-status`} className="sr-only">{layer.detail}. This overlay is disabled until provider authorization is complete.</small></label>)}</div></div><p>Imagery and overlays provide operational context; they do not confirm field safety or route clearance.</p></div>}</section>} />
    ...
    <div className="command-map-quicklinks" aria-label="Operational workstreams"><button type="button" onClick={() => onNavigate("Live SOS")}>Incidents <b>{summary.metrics.untriaged_sos}</b></button><button type="button" onClick={() => onNavigate("Fleet & Responder Safety")}>Field response <b>{groups.groups.filter((group) => group.status === "en_route" || group.status === "deployed").length}</b></button><button type="button" onClick={() => onNavigate("Evacuation Centers")}>Community safety <b>{summary.metrics.open_centers}</b></button><button type="button" onClick={() => onNavigate("DRRMO Intelligence")}>Intelligence</button></div>
