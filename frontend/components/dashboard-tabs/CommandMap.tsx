...
export function CommandMapView({ summary, gis, groups, health, user, connection, appearance, onAppearanceChange, onSelect, onNavigate, onAction, onRefresh, error }: { summary: DashboardSummary; gis: GisMapSnapshot; groups: ResponseGroupSnapshot; health: FeedHealth[]; user: UserIdentity | null; connection: "live" | "cached"; appearance: "dark" | "light"; onAppearanceChange: () => void; onSelect: (incident: SosIncident) => void; onNavigate: (tab: CommandCenterTab) => void; onAction: OperationalAction; onRefresh: () => void; error: string | null }) {
  ...
  return <main className={`command-map-shell appearance-${appearance}`} aria-label="Command Map Live Operations">
    ...
    <header className="command-map-topbar">
      ...
      <div className="command-map-session"><div className={`system-health ${systemHealthy ? "healthy" : "degraded"}`}><span />{systemHealthy ? "Systems connected" : "Review source health"}</div><button className="command-avatar" type="button" aria-label="Current operator profile">{user?.display_name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase() || "DR"}</button><button className="appearance-toggle" type="button" onClick={onAppearanceChange} aria-pressed={appearance === "dark"} aria-label={appearance === "dark" ? "Switch to light mode" : "Switch to dark mode"} title={appearance === "dark" ? "Switch to light mode" : "Switch to dark mode"}><span aria-hidden="true">{appearance === "dark" ? "☀" : "☾"}</span><b>{appearance === "dark" ? "Light" : "Dark"}</b></button></div>
    </header>
