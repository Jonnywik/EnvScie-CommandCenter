...
import { type DispatchLifecycleAssignment, type DispatchLifecycleSnapshot, type GisMapSnapshot, type GisResource, type OptimizedRoute, type ResponseGroup, type ResponseGroupSnapshot, getDispatchLifecycle, optimizeGisRoute, sendCoordinationCommunication, transitionDispatchLifecycle } from "../../lib/api";
...
export function FleetResponderSafetyView({ groups, gis, appearance, onAppearanceChange, onAction, onRefresh, onReturn, onNavigate, selectedGroupId, onSelectedGroupChange }: { groups: ResponseGroupSnapshot; gis: GisMapSnapshot; appearance: AppearanceMode; onAppearanceChange: () => void; onAction: OperationalAction; onRefresh: () => Promise<void>; onReturn: () => void; onNavigate: (tab: CommandCenterTab) => void; selectedGroupId?: string | null; onSelectedGroupChange?: (groupId: string) => void }) {
  ...
  const activeDispatch = selectedDispatches.find((assignment) => !["cancelled", "closed"].includes(assignment.status)) || null;
  const recordFieldAcknowledgement = async (assignment: DispatchLifecycleAssignment) => {
    setLifecycleBusy(true); setActionStatus(null);
    try {
      const updated = await transitionDispatchLifecycle(assignment.assignment_id, { action: "acknowledge", note: "Field unit acknowledgement reported to Fleet Safety operator." });
      await onAction("fleet.dispatch_acknowledgement_recorded", "response_group_assignment", assignment.assignment_id, `Recorded reported unit acknowledgement for ${selected?.group.name || assignment.group_id}; notification receipt was not used as acknowledgement.`);
      setActionStatus(`Dispatch lifecycle updated to ${updated.status}. Reported acknowledgement remains distinct from notification delivery and field safety.`);
      setDispatchLifecycle(await getDispatchLifecycle()); await onRefresh();
    } catch (error) { setActionStatus(error instanceof Error ? error.message : "Field acknowledgement could not be recorded."); }
  ...
  return <div className="workspace-navigation-shell"><CommandCenterNavigation activeTab="Fleet & Responder Safety" onNavigate={onNavigate} /><main className="field-response-fleet min-h-screen bg-slate-950 p-3 font-sans text-slate-100 sm:p-5" aria-label="Field Response fleet safety dashboard">
    <CommandCenterHeader title="Field Response / Fleet Safety" context="COMMAND CENTER · ASSET READINESS AND FIELD PROTECTION" appearance={appearance} onAppearanceChange={onAppearanceChange} operatorLabel="Duty officer · field response" syncLabel="Current fleet and responder snapshot" onReturn={onReturn} />
    ...
    {selected && <section className="mx-auto mt-4 max-w-[1800px] rounded-2xl border border-white/10 bg-slate-900/90 p-4 shadow-command" aria-label="Selected unit dispatch lifecycle"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="m-0 text-[10px] font-bold tracking-[0.18em] text-amber-300">DISPATCH LIFECYCLE · SELECTED UNIT</p><h2 className="m-0 mt-1 text-base font-semibold text-white">{selected.group.call_sign || selected.group.name}</h2></div>{activeDispatch && <span className="rounded-full border border-amber-300/40 bg-amber-300/10 px-3 py-1 text-xs font-bold capitalize text-amber-100">{activeDispatch.status.replaceAll("_", " ")}</span>}</div>{activeDispatch ? <div className="mt-3 flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-slate-950/60 p-3"><div className="min-w-0 flex-1"><strong className="block text-sm text-white">{activeDispatch.assignment_note || `Assignment to ${activeDispatch.target_type}`}</strong><span className="mt-1 block text-xs leading-5 text-slate-400">Lifecycle records human decisions and reported acknowledgement; they do not prove notification delivery, route clearance, or field safety.</span></div>{activeDispatch.status === "pending_confirmation" && <button type="button" onClick={() => onNavigate("Incident Triage")} className="rounded-lg bg-orange-400 px-3 py-2 text-xs font-bold text-slate-950">Open Triage confirmation</button>}{activeDispatch.status === "confirmed" && <button type="button" disabled={lifecycleBusy} onClick={() => void recordFieldAcknowledgement(activeDispatch)} className="rounded-lg bg-teal-300 px-3 py-2 text-xs font-bold text-slate-950 disabled:cursor-not-allowed disabled:bg-slate-700">Record reported acknowledgement</button>}<ol className="w-full space-y-1 border-t border-white/10 pt-3 text-xs text-slate-400">{activeDispatch.events.map((event) => <li key={event.id}><strong className="capitalize text-slate-200">{event.to_status.replaceAll("_", " ")}</strong> · {new Date(event.occurred_at).toLocaleString()} {event.note ? `· ${event.note}` : ""}</li>)}</ol></div> : <p className="mb-0 mt-3 text-sm text-slate-400">No active human-confirmed dispatch lifecycle record is linked to this unit. A pending proposal does not change unit movement until a coordinator confirms it in Incident Triage.</p>}</section>}
