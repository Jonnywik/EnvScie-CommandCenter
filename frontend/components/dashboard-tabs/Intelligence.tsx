...
export function IntelligenceDashboardView({ health, connection, summary, appearance, onAppearanceChange, onAction, onReturn, onNavigate }: { health: FeedHealth[]; connection: "live" | "cached"; summary: DashboardSummary; appearance: AppearanceMode; onAppearanceChange: () => void; onAction: OperationalAction; onReturn: () => void; onNavigate: (tab: CommandCenterTab) => void }) {
  ...
  const [incidentHandover, setIncidentHandover] = useState<IncidentRecord[]>([]);
  ...
  const refreshLedger = useCallback(async () => {
    ...
    try { const [nextEvents, nextIncidents, nextReadiness] = await Promise.all([getAuditEvents(200), getIncidents(), getOperationsReadiness()]); setEvents(nextEvents); setIncidentHandover(nextIncidents.incidents); setReadiness(nextReadiness); }
  ...
  const cards = [
    ...
    { label: "Release configuration", detail: readiness ? `${Object.values(readiness.checks).filter(Boolean).length}/${Object.keys(readiness.checks).length} required categories ready` : "Readiness status unavailable", state: readiness?.release_ready ? "green" : readiness ? "yellow" : "red", note: readiness ? readiness.decision_limit : "The non-sensitive production readiness check could not be loaded." },
  ] as const;
  const makeCsv = () => [
  ...
  return <div className="workspace-navigation-shell"><CommandCenterNavigation activeTab="DRRMO Intelligence" onNavigate={onNavigate} /><main className="min-h-screen bg-slate-950 p-3 font-sans text-slate-100 sm:p-5" aria-label="DRRMO Intelligence Audit and Analytics dashboard">
    <CommandCenterHeader title="DRRMO Intelligence / Audit & Analytics" context="COMMAND CENTER · POST-INCIDENT REVIEW AND ACCOUNTABILITY" appearance={appearance} onAppearanceChange={onAppearanceChange} operatorLabel="Duty officer · intelligence" syncLabel="Current audit and analytics snapshot" onReturn={onReturn} />
    ...
    <section className="mx-auto mb-4 grid max-w-[1800px] gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.42fr)]"><section className="rounded-2xl border border-amber-300/25 bg-slate-900/90 p-4 shadow-command" aria-label="Incident follow-up handover"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="m-0 text-[10px] font-bold tracking-[0.18em] text-amber-300">SHIFT HANDOVER · INCIDENT FOLLOW-UP</p><h2 className="m-0 mt-1 text-lg font-semibold text-white">Unresolved incident command records</h2><p className="mb-0 mt-1 text-xs text-slate-400">Review ownership and due dates at handover. This list does not alter records, dispatches, or closures.</p></div><span className="rounded-full border border-amber-300/35 bg-amber-300/10 px-3 py-1 text-xs font-bold text-amber-100">{incidentHandover.filter((item) => item.status !== "closed").length} open</span></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{incidentHandover.filter((item) => item.status !== "closed").slice(0, 6).map((item) => <article key={item.id} className="rounded-xl border border-white/10 bg-slate-950/60 p-3"><div className="flex items-start justify-between gap-2"><strong className="text-sm text-white">{item.emergency_type} · {item.barangay}</strong><span className="badge advisory capitalize">{item.status}</span></div><p className="mb-0 mt-2 line-clamp-2 text-xs leading-5 text-slate-300">{item.summary}</p><span className="mt-3 block text-xs text-slate-400">Owner: {item.follow_up_owner || "unassigned"} · due {item.follow_up_due_at ? formatPst(item.follow_up_due_at) : "not scheduled"}</span></article>)}{!incidentHandover.some((item) => item.status !== "closed") && <div className="rounded-xl border border-dashed border-white/15 p-4 text-sm text-slate-400">No open incident command records are available for handover.</div>}</div></section><aside className="rounded-2xl border border-amber-300/25 bg-slate-900/90 p-4 shadow-command" aria-label="Production release readiness"><p className="m-0 text-[10px] font-bold tracking-[0.18em] text-amber-300">PRODUCTION RELEASE READINESS</p><h2 className="m-0 mt-1 text-lg font-semibold text-white">{readiness?.release_ready ? "Configuration gate ready" : "Configuration gate blocked"}</h2><p className="mb-0 mt-2 text-xs leading-5 text-slate-400">{readiness?.decision_limit || "Readiness is being checked without exposing provider endpoints or credentials."}</p><ul className="mb-0 mt-3 space-y-2 pl-4 text-xs leading-5 text-amber-100">{(readiness?.blockers || ["Readiness status could not be loaded; confirm release prerequisites through approved infrastructure records."]).slice(0, 4).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></aside></section>
...
export function TriageDrawer({ incident, onClose, onUpdated, onAction }: { incident: SosIncident; onClose: () => void; onUpdated: (incident: SosIncident) => void; onAction: OperationalAction }) {
  ...
  const openDispatchSelector = async () => {
    ...
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Response teams could not be loaded for this triage record."); }
